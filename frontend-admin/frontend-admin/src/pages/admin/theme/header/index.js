...
import "./style.scss";
import { FaCircleChevronDown, FaCircleChevronUp, FaListUl, FaUser } from "react-icons/fa6";
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const response = await axios.get("http://localhost:8080/api/users/67bcad47a8c08f1a84f6aea1", {
                    headers: {
    ...
    return (
        <>
            ...
            <div ref={sidebarRef} className={`admin_sidebar ${isOpen ? "open" : ""}`} {...props}>
                ...
                <h2 className="admin_sidebar-title"><FaUser /> {username || "Đang tải..."}</h2>
                <div className="menu_wrapper_navbar">
                    <ul>
                        {navItems.map((item, index) => (
                            <li
                                key={index}
                                className={`${item.isShowSubmenu ? "show_submenu" : ""} ${activePath === item.path ? "active" : ""
                                    }`}
                            >
                                {item.child ? (
                                    <>
